        ...
        return new Promise((resolve, reject) => {
          return axios(options)
            .then(function ({ status, data }) {
              log.debug('MarketoClient: response status = ' + status)
              log.debug('MarketoClient: response data   = ' + JSON.stringify(data))
              if (data.success === false && _.has(data, 'errors')) {
                log.debug('MarketoClient: request failed: ', data);
...
Connection.prototype.getOAuthToken = function (force) {
  ...
  if (force || this._tokenData == null) {
    ...
    log.debug('MarketoClient: using existing token: ', this._tokenData);
